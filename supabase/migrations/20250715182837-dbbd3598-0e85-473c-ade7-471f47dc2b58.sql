-- Create table for galactic systems/planets
...
-- Create policy for public read access (anyone can view the galactic database)
CREATE POLICY "Allow public read access to galactic systems" 
ON public.galactic_systems 
FOR SELECT 
USING (true);

-- Create policy for authenticated users to insert new systems
CREATE POLICY "Authenticated users can insert galactic systems" 
ON public.galactic_systems 
FOR INSERT 
WITH CHECK (auth.role() = 'authenticated');

-- Create policy for authenticated users to update systems
CREATE POLICY "Authenticated users can update galactic systems" 
ON public.galactic_systems 
FOR UPDATE 
USING (auth.role() = 'authenticated');

-- Create indexes for better performance
CREATE INDEX idx_galactic_systems_region ON public.galactic_systems(region);
CREATE INDEX idx_galactic_systems_sector ON public.galactic_systems(sector);
CREATE INDEX idx_galactic_systems_grid ON public.galactic_systems(grid_coordinates);
CREATE INDEX idx_galactic_systems_name ON public.galactic_systems(name);
...
-- Insert the provided planetary data
...
('Ahch-To', NULL, 'Unknown Regions', 'F-13'),
('Ahn Krantarium', 'Esuain', 'Mid Rim', 'P-7'),
('Ai''ken Prime', NULL, 'Inner Rim', 'O-10'),
('Aida', 'Aida', 'Mid Rim', 'Q-13'),
('Aikhibba', 'Fei Hu', 'Mid Rim', 'P-13'),
('Ailon', NULL, 'Inner Rim', 'N-12'),
